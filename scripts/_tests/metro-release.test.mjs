...
test('renderer emits four static accessible private drafts',async t=>{const output=await mkdtemp(path.join(tmpdir(),'barton-render-'));t.after(()=>rm(output,{recursive:true,force:true}));const pages=await renderDrafts({output});assert.equal(pages.size,4);for(const [file,html]of pages){const doc=htmlDocument(html);assert.equal(doc.noindex,true);assert.equal(doc.h1.length,1);assert.deepEqual(doc.canonical,[`${origin}/${file}`]);assert.match(doc.visibleText,/Local editorial draft/);assert.ok(doc.links.includes('#main-content'));}assert.match(htmlDocument(pages.get('austin.html')).visibleText,/buyer-paid shipping/);await renderDrafts({output,check:true});await writeFile(path.join(output,'houston.html'),'stale');await assert.rejects(renderDrafts({output,check:true}),/stale draft/);});
