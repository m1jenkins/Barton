...
`npm test` runs the backend, advertising, and buying-brief tests. `npm run check:api` and `npm run check:buying` check syntax and site metadata/assets. The homepage routes are `/#conversation` and `/#brief`; pricing remains `/schedule.html`.
...
Daisy's photographs and fonts were reused from the supplied reference project. Font licenses are included in `assets/buying/fonts/`. Responsive coastal-photo derivatives preserve the original composition. Styles are loaded only by the redesigned pages; the added Drive Right rules are scoped to `.dr`.
