import { fields, normalizeAnswer, parseConversation, nextField, isComplete, isConcrete, choicesFor, restoredPriorities } from './intake.js';
...
function currentField() { return isComplete(brief.answers) && !optionalQuestions ? null : nextField(brief.answers, brief.skipped); }
function renderConversation() {
  const field = currentField();
  const completed = fields.filter(f => f.required && brief.answers[f.key]).length;
  $('#step-label').textContent = `${completed} of 3 essentials`;
  $('#intake-progress').value = completed;
  const rows = fields.filter(f => brief.answers[f.key] || brief.skipped[f.key]);
  $('#messages').innerHTML = rows.map(f => `<div class="message user"><div class="bubble"><p>${escape(brief.answers[f.key] || 'Skipped for now')}</p><button class="bubble-edit" data-edit="${f.key}" aria-label="Edit ${escape(f.label)}">${icon('edit')}</button></div><span class="message-label">${escape(f.label)}</span></div>`).join('') + `<div class="message assistant"><p>${escape(field?.question || 'That’s enough to get started. Your brief is ready to review.')}</p></div>`;
  $('#choices').innerHTML = field ? choicesFor(field, brief.answers).map(choice => `<button class="choice-button" type="button" data-choice="${escape(choice)}">${escape(choice)}</button>`).join('') : '';
  $('#choices').hidden = !field;
  $('#skip-detail').hidden = !field || field.required;
  ...
  $('#answer-label').textContent = field?.question || 'Your answer';
  $('#input-hint').textContent = field?.hint || 'You can edit any answer.';
  $('.conversation-controls').hidden = !field;
  $('#brief-ready').hidden = !isComplete(brief.answers);
  $('#add-details').hidden = !nextField(brief.answers, brief.skipped) || optionalQuestions;
  requestAnimationFrame(() => { const log = $('#messages'); log.scrollTo({ top:log.scrollHeight, behavior:reduced() ? 'instant' : 'smooth' }); });
}
...
function renderBrief() {
  ...
  $('#brief-details').innerHTML = fields.filter(f => f.key !== 'vehicle' && brief.answers[f.key]).map(f => `<div class="brief-detail"><dt>${escape(f.label)}</dt><dd><button data-edit="${f.key}" aria-label="Edit ${escape(f.label)}">${escape(brief.answers[f.key])}</button></dd></div>`).join('');
}
...
if (home) {
  ...
  $('#add-details').addEventListener('click', () => { optionalQuestions = true; renderConversation(); input.focus(); });
  ...
  $('#edit-fields').addEventListener('click', e => {
    ...
    if (skip) { const row = skip.closest('[data-field]'); row.dataset.skipped = 'true'; row.querySelector('input').value = ''; skip.textContent = 'Skipped for now'; }
  });
  ...
  $('#edit-form').addEventListener('submit', e => {
    ...
    try {
      $('#edit-fields').querySelectorAll('[data-field]').forEach(row => {
        const key = row.dataset.field, value = row.querySelector('input').value.trim(), field = fields.find(f => f.key === key);
...
if (paidBrief) {
  ...
  function carryOver() {
    ...
    for (const [name, text] of Object.entries(onboardingValues(value, contact))) {
      ...
      if (control && !control.value && text) { if (name === 'year_min') control.min = '1886'; control.value = text; }
    }
